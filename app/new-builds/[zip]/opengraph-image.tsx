...
import { ZIP_META_MAP, NASHBUILDS_ZIPS } from '@/lib/nashbuilds-zips'

export const runtime = 'edge'
export const alt = 'NashBuilds ZIP Page'
export const size = { width: 1200, height: 630 }
export const contentType = 'image/png'

export function generateImageMetadata() {
  return NASHBUILDS_ZIPS.map((z) => ({
    id: z.zip,
    alt: `New Construction in ${z.zip} ${z.name} — NashBuilds`,
    size,
    contentType,
  }))
}
  ...
  return new ImageResponse(
    (
      <div style={{ background: '#000', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui' }}>
        <div style={{ fontSize: '28px', color: '#C6C4C4', letterSpacing: '0.15em', textTransform: 'uppercase' }}>NashBuilds</div>
        ...
        <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.4)', marginTop: '24px' }}>House Haven Realty &middot; househavenrealty.com/new-builds/{params.zip}</div>
      </div>
    ),
    { ...size },
  )
